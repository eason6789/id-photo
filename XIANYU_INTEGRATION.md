...
### 1. 服务器信息
- **服务器 IP**: 119.29.178.222
- **SSH 账号**: root
- **SSH 密码**: *7;Vf3@BkrzD,g+u
- **SSH 连接命令**: 
  ```bash
  sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222
  ```
...
# 检查 MySQL 数据库连接
mysql -u root -p'YOUR_DB_PASSWORD' -e "USE wish_plan; SHOW TABLES;"
```
...
```bash
# 检查服务状态
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "systemctl status photo-service"

# 检查 Nginx 配置
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "grep -A 5 'location /api/' /etc/nginx/conf.d/tuteng3.site.conf"
...
```bash
# 如果 proxy_pass 端口错误
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "sed -i 's|proxy_pass http://127.0.0.1:5000|proxy_pass http://127.0.0.1:8080|g' /etc/nginx/conf.d/tuteng3.site.conf && nginx -t && systemctl reload nginx"
```
...
```bash
# 查看服务日志
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "journalctl -u photo-service -n 50 | grep -i 'facechain\|error\|panic'"
```
...
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "grep -A 3 '将毫米转换为像素' /root/photo-service/main.go"
...
```bash
# 测试数据库连接
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "mysql -u root -p'YOUR_DB_PASSWORD' -e 'SELECT 1' 2>&1"

# 检查 MySQL 服务状态
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "systemctl status mysqld"
```
...
```sql
-- 连接到数据库
mysql -u root -p'YOUR_DB_PASSWORD' wish_plan
...
# 查询数据库
mysql -u root -p'YOUR_DB_PASSWORD' wish_plan -e "SELECT * FROM tokens ORDER BY created_at DESC LIMIT 10;"
```
