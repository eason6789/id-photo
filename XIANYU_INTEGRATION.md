...
# 检查 MySQL 数据库连接
mysql -u root -p'OpenClaw2026!' -e "USE wish_plan; SHOW TABLES;"
```
...
```bash
# 测试数据库连接
sshpass -p '*7;Vf3@BkrzD,g+u' ssh root@119.29.178.222 "mysql -u root -p'OpenClaw2026!' -e 'SELECT 1' 2>&1"
...
```sql
-- 连接到数据库
mysql -u root -p'OpenClaw2026!' wish_plan
...
# 查询数据库
mysql -u root -p'OpenClaw2026!' wish_plan -e "SELECT * FROM tokens ORDER BY created_at DESC LIMIT 10;"
```
